...
// Update user
router.put('/users/:userId', authenticateAdmin, asyncHandler(async (req, res) => {
  const { userId } = req.params;
...
router.delete('/users/:userId', authenticateAdmin, asyncHandler(async (req, res) => {
  const { userId } = req.params;
...
// Get all farms
router.get('/farms', authenticateAdmin, asyncHandler(async (req, res) => {
...
router.get('/products', authenticateAdmin, asyncHandler(async (req, res) => {
...
router.post('/impersonate/:userId', authenticateAdmin, asyncHandler(async (req, res) => {
  const { userId } = req.params;
...
router.get('/store-products', authenticateAdmin, asyncHandler(async (req, res) => {
  const { page = 1, limit = 20, search, farm_id } = req.query;
  const skip = (parseInt(page) - 1) * parseInt(limit);

  const { db } = await connectToDatabase();
  const { store_products: storeProducts } = await getCollections(db);
