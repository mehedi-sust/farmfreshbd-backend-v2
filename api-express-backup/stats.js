...
async function calculateFarmStats(farmIdObj, db) {
  const { products, expenses, investments, sales } = getCollections(db);
...
app.post('/farm/:farm_id/update_stats_data', authenticate, asyncHandler(async (req, res) => {
  const { farm_id } = req.params;
...
app.get('/farm/:farm_id/monthly_financial', authenticate, asyncHandler(async (req, res) => {
  const { farm_id } = req.params;
...
app.get('/farm/:farm_id/current_year_profit', authenticate, asyncHandler(async (req, res) => {
  const { farm_id } = req.params;
  ...
  const { expenses, sales } = getCollections(db);
  const farmIdObj = toObjectId(farm_id);
...
app.get('/farm/:farm_id/profit_over_time', authenticate, asyncHandler(async (req, res) => {
  const { farm_id } = req.params;
...
app.get('/farm/:farm_id/batch_summaries', authenticate, asyncHandler(async (req, res) => {
  const { farm_id } = req.params;
  ...
  const { products, expenses, productBatches, sales } = getCollections(db);
  const farmIdObj = toObjectId(farm_id);
...
app.get('/dashboard', authenticate, asyncHandler(async (req, res) => {
  ...
  const { products, expenses, investments, sales, orders } = getCollections(db);
